...
namespace Mavenbird\Blog\Model\Import;
...
use Mavenbird\Blog\Model\Config\Source\AuthorType;
use Mavenbird\Blog\Model\Config\Source\Comments\Status;
...
/**
 * Class MavenBirdM2
 * @package Mavenbird\Blog\Model\Import
 */
class MavenBirdM2 extends AbstractImport
{
    /**
     * Mavenbird Post table name
     *
     * @var string
     */
    const TABLE_POST = 'mavenbird_blog_post';

    /**
     * Mavenbird Related Post table name
     *
     * @var string
     */
    const TABLE_POST_RELATED = 'mavenbird_blog_post_relatedpost';

    /**
     * Mavenbird Tag table name
     *
     * @var string
     */
    const TABLE_TAG = 'mavenbird_blog_tag';

    /**
     * Mavenbird Post-Tag Relationship table name
     *
     * @var string
     */
    const TABLE_POST_TAG = 'mavenbird_blog_post_tag';

    /**
     * Mavenbird Category table name
     *
     * @var string
     */
    const TABLE_CATEGORY = 'mavenbird_blog_category';

    /**
     * Mavenbird Post-Category Relationship table name
     *
     * @var string
     */
    const TABLE_POST_CATEGORY = 'mavenbird_blog_post_category';

    /**
     * Mavenbird Comment table name
     *
     * @var string
     */
    const TABLE_COMMENT = 'mavenbird_blog_comment';

    /**
     * Mavenbird Customer table name
     *
     * @var string
     */
    ...
    /**
     * Mavenbird Admin user table name
     *
     * @var string
     */
    ...
    public function run($data, $connection)
    {
        ...
        if ($this->_importPosts($data, $connection) && $data['type'] == $this->_type['mavenbird']) {
    ...
    protected function _importPosts($data, $connection)
    {
        ...
        if ($isReplace && isset($sourceItems)) {
            ...
            foreach ($postModel->getCollection() as $item) {
                if ($item->getImportSource() != null) {
                    ...
                    if ($importType == $this->_type['mavenbird']) {
            ...
            while ($topic = mysqli_fetch_assoc($result)) {
                if ($topicModel->getResource()->isImported($importSource, $topic['post_id']) == false) {
                    try {
                        $topicModel->setData([
                            'name'          => 'mavenbird-topic-' . $topicCount,
                            'enabled'       => 1,
                            'store_ids'     => $this->_storeManager->getStore()->getId(),
             ...
             */
            $topicPostTable = $this->_resourceConnection->getTableName('mavenbird_blog_post_topic');
        ...
        $this->_registry->register('mavenbird_import_post_ids_collection', $oldPostIds);
        $this->_registry->register('mavenbird_import_post_statistic', $statistics);
    ...
    protected function _importTags($data, $connection)
    {
        ...
        if ($isReplace && isset($sourceItems)) {
            ...
            $tagPostTable = $this->_resourceConnection->getTableName('mavenbird_blog_post_tag');
            $sqlTagPost   = "SELECT * FROM `" . $data['table_prefix'] . self::TABLE_POST_TAG . "` ";
            $result       = mysqli_query($connection, $sqlTagPost);
            $oldPostIds   = $this->_registry->registry('mavenbird_import_post_ids_collection');
        ...
        $this->_registry->register('mavenbird_import_tag_statistic', $statistics);
    }
    ...
    protected function _importCategories($data, $connection)
    {
        ...
        if ($isReplace && isset($sourceItems)) {
             ...
             */
            $categoryPostTable = $this->_resourceConnection->getTableName('mavenbird_blog_post_category');
        ...
        $this->_registry->register('mavenbird_import_category_statistic', $statistics);
    }
    ...
    protected function _importComments($data, $connection)
    {
            ...
            foreach ($sourceItems as $comment) {
                ...
                if ($comment['is_imported']) {
                    ...
                    $this->_resourceConnection->getConnection()
                        ->update(
                            $this->_resourceConnection->getTableName('mavenbird_blog_comment'),
                            [
        ...
        $this->_registry->register('mavenbird_import_comment_statistic', $statistics);
    }
    ...
    protected function _importAuthors($data, $connection)
    {
        ...
        foreach ($updateData as $postId => $authorId) {
            ...
            $this->_resourceConnection->getConnection()
                ->update(
                    $this->_resourceConnection->getTableName('mavenbird_blog_post'),
                    ['author_id' => $authorId],
                    $where
                );
        }
        $statistics = $this->_getStatistics('authors', $this->_successCount, $this->_errorCount, $this->_hasData);
        $this->_registry->register('mavenbird_import_user_statistic', $statistics);
    }
    ...
    private function _updatePosts($post, $where)
    {
        ...
        $this->_resourceConnection->getConnection()->delete($this->_resourceConnection
            ->getTableName('mavenbird_blog_post_category'), $where);
        $this->_resourceConnection->getConnection()->delete($this->_resourceConnection
            ->getTableName('mavenbird_blog_post_tag'), $where);
    }
    ...
    private function _updateTags($tag, $where)
    {
        $this->_resourceConnection->getConnection()
            ->update(
                $this->_resourceConnection->getTableName('mavenbird_blog_tag'),
                [
                    'name'             => $tag['name'],
                    'url_key'          => $tag['url_key'],
                    'meta_robots'      => $tag['meta_robots'],
    ...
    private function _updateCategories($category, $where)
    {
        $this->_resourceConnection->getConnection()
            ->update(
                $this->_resourceConnection->getTableName('mavenbird_blog_category'),
                [
                    'name'             => $category['name'],
                    'url_key'          => $category['url_key'],
                    'meta_robots'      => $category['meta_robots'],
